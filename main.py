...
messagebox.showinfo("使用说明", "1. 点击按钮绘制图形\n2. 点击清空按钮清空屏幕\n3. 点击退出按钮退出程序\n\n注意：绘制图形时请勿频繁点击按钮，否则可能会导致程序崩溃。\n注意：绘制图形时请勿频繁点击按钮，否则可能会导致程序崩溃。\n\n使用时请将点击“确定”按钮后的按钮界面拖至左半屏，turtle界面拖至右半屏，以免遮挡！\n使用时请将点击“确定”按钮后的按钮界面拖至左半屏，turtle界面拖至右半屏，以免遮挡！！\n使用时请将点击“确定”按钮后的按钮界面拖至左半屏，turtle界面拖至右半屏，以免遮挡！！！")
...
# 创建主窗口
root = tk.Tk()
root.title("画图工具大全v1.0.1")

btn_rectangle = tk.Button(root, text="绘制爱心", command=draw_heart)
...
btn_hkdtx = tk.Button(root, text="漂亮的圆形", command=hkdtx)
btn_hkdtx.pack(pady=5)
'''
btn_mandelbrot = tk.Button(root, text="绘制曼德布罗集", command=draw_mandelbrot)
btn_mandelbrot.pack(pady=5)
'''

btn_wnk = tk.Button(root, text="蜗牛壳", command=wnk)
btn_wnk.pack(pady=5)
